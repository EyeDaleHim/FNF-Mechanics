...
class ControlsSubState extends MusicBeatSubstate
{
	...
	var optionShit:Array<Dynamic> = [
		...
		['Down', 'ui_down'], ['Up', 'ui_up'], ['Right', 'ui_right'], [''], ['OTHERS'], ['Interact', 'interact'], ['Dodge', 'dodge'], ['Reset', 'reset'],
		['Accept', 'accept'], ['Back', 'back'], ['Pause', 'pause'], [''], ['VOLUME'], ['Mute', 'volume_mute'], ['Up', 'volume_up'], ['Down', 'volume_down'],
		[''], ['DEBUG'], ['Key 1', 'debug_1'], ['Key 2', 'debug_2']];
	...
	override function update(elapsed:Float)
	{
		...
		else
		{
			...
			// var mousePressed:Bool = FlxG.mouse.justPressed;
			// var mousePressedRight:Bool = FlxG.mouse.justPressedRight;

			var mousePressed:Bool = false;
			var mousePressedRight:Bool = false;

			if (((mousePressed || mousePressedRight) && curSelected < 6) || keyPressed > -1)
			{
				var keysArray:Array<FlxKey> = ClientPrefs.keyBinds.get(optionShit[curSelected][1]);
				if ((mousePressed || mousePressedRight))
				{
					if (mousePressed)
						keysArray[curAlt ? 1 : 0] = -4;
					else if (mousePressedRight)
						keysArray[curAlt ? 1 : 0] = -5;
				}
				else
