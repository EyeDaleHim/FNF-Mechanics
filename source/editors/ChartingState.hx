...
@:access(flixel.system.FlxSound._sound) @:access(openfl.media.Sound.__buffer) class ChartingState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		['Freeplay Shader', "Value 1: Saturation\nValue 2: Time to Change."],
		[
			'Change Scroll Speed',
	...
	**/
	...
	override function create()
	{
		...
		dragSprite = new FlxSprite().makeGraphic(1, 1, FlxColor.CYAN);
		dragSprite.scrollFactor.set();
		dragSprite.visible = false;
		dragSprite.alpha = 0.2;
		add(dragSprite);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText))
		{
			...
			else if (curSelectedNote != null)
			{
				...
				{
					if (curSelectedNote[1][curEventSelected][1] != null)
						curSelectedNote[1][curEventSelected][1] = value1InputText.text;
					updateGrid();
				}
				else if (sender == value2InputText)
				{
					if (curSelectedNote[1][curEventSelected][2] != null)
	...
	var selectMultipleOnce:Bool = false;
	var dragSprite:FlxSprite = null;
	var originalPos:FlxPoint = FlxPoint.get();
	var offsetPos:FlxPoint = FlxPoint.get();
	...
	override function update(elapsed:Float)
	{
		...
		selectMultiple = dragSprite.visible = (holdTime > 0.5);
		if (FlxG.mouse.pressed)
			holdTime += elapsed;
		else
			holdTime = 0;

		if (!selectMultipleOnce && selectMultiple)
		{
			selectMultipleOnce = true;
			originalPos = FlxG.mouse.getScreenPosition();
			dragSprite.setPosition(FlxG.mouse.getScreenPosition().x, FlxG.mouse.getScreenPosition().y);
			offsetPos.set();
		}

		if (selectMultiple)
		{
			if (FlxG.mouse.getScreenPosition().x < dragSprite.x)
			{
				offsetPos.x = FlxG.mouse.getScreenPosition().x - originalPos.x;
			}

			if (FlxG.mouse.getScreenPosition().y < dragSprite.y)
			{
