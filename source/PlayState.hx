...
class PlayState extends MusicBeatState
{
		...
		if (!stageData.hide_girlfriend)
		{
			...
			if (gfVersion == 'pico-speaker')
			{
				if (!ClientPrefs.lowQuality)
				{
					...
					var timeSinceLast:Float = -1;

					for (i in 0...TankmenBG.animationNotes.length)
					{
						if (FlxG.random.bool(16 + (((timeSinceLast / Conductor.stepCrochet) / 24))))
						{
							var tankBih = tankmanRun.recycle(TankmenBG);
							tankBih.strumTime = TankmenBG.animationNotes[i][0];
							if (timeSinceLast == -1)
								timeSinceLast = tankBih.strumTime;
							else
								timeSinceLast = tankBih.strumTime - timeSinceLast;
	...
	function tankIntro()
	{
		...
		switch (songName)
		{
			case 'ugh':
				...
				{
					wellWellWell.onComplete = wellWellWellFunc;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (floorSus > 0)
				{
					for (susNote in 0...floorSus + 1)
					{
						...
						sustainNote.parent = cast swagNote;
						swagNote.sustainChilds.push(sustainNote);
			...
			for (j in [false, true])
			{
				...
				if (section.sectionNotes.length < 4)
					hitSectionMulti = 0.04;
				for (i in 0...section.lengthInSteps)
				{
					if (FlxG.random.bool((Math.min(MechanicManager.mechanics['hurt_note'].points * FlxMath.remapToRange(section.lengthInSteps, 0, 16, 1,
						6) / noteData.length * 0.2, 1))
						+ getChance(0)))
					{
						var hurtNote:Note = new Note(sectionStartTime + (Conductor.stepCrochet * i), FlxG.random.int(0, 3), null, false);
						hurtNote.strumTime -= 2; // prioritize chart notes
						hurtNote.autoGenerated = true;
						hurtNote.earlyHitMult = 0.5;
						hurtNote.mustPress = hurtNote.formerPress = j;
						if (playBothMode)
							hurtNote.mustPress = true;
						hurtNote.noteType = 'Hurt Note';
						hurtNote.scrollSpeed = songSpeed;
						hurtNote.scrollFactor.set();
						unspawnNotes.push(hurtNote);
						spawnedNotes.push(hurtNote);
						weightedChances[0] = 0;
					}
					else
					{
						weightedChances[0] += FlxG.random.float(0, FlxMath.remapToRange(MechanicManager.mechanics['hurt_note'].points, 0, 20, 0, 2));
					}
					if (FlxG.random.bool(((Math.min(MechanicManager.mechanics['kill_note'].points * FlxMath.remapToRange(section.lengthInSteps, 0, 16, 1,
						6) / noteData.length * 0.2, 1))
						+ getChance(1)) * 0.5))
					{
						var killNote:Note = new Note(sectionStartTime + (Conductor.stepCrochet * i), FlxG.random.int(0, 3), null, false);

						killNote.strumTime -= 2; // prioritize chart notes
						killNote.autoGenerated = true;
						killNote.earlyHitMult = 0.4;
						killNote.mustPress = killNote.formerPress = j;
						if (playBothMode)
							killNote.mustPress = true;
						killNote.noteType = 'Kill Note';
						killNote.scrollSpeed = songSpeed;
						killNote.scrollFactor.set();
						unspawnNotes.push(killNote);
						spawnedNotes.push(killNote);
						weightedChances[1] = 0;
					}
					else
					{
						weightedChances[1] += FlxG.random.float(0, FlxMath.remapToRange(MechanicManager.mechanics['kill_note'].points, 0, 20, 0, 2)) * 0.4;
					}
					if (FlxG.random.bool(((Math.min(MechanicManager.mechanics['burst_note'].points * FlxMath.remapToRange(section.lengthInSteps, 0, 16, 1,
						6) / noteData.length * 0.2, 1))
						+ getChance(2)) * 0.9))
					{
						var burstNote:Note = new Note(sectionStartTime + (Conductor.stepCrochet * i), FlxG.random.int(0, 3), null, false);

						burstNote.strumTime -= 2; // prioritize chart notes
						burstNote.autoGenerated = true;
						burstNote.earlyHitMult = 0.4;
						burstNote.mustPress = burstNote.formerPress = j;
						if (playBothMode)
							burstNote.mustPress = true;
						burstNote.noteType = 'Burst Note';
						burstNote.scrollSpeed = songSpeed;
						burstNote.scrollFactor.set();
						unspawnNotes.push(burstNote);
						spawnedNotes.push(burstNote);
						weightedChances[2] = 0;
					}
					else
					{
						weightedChances[2] += FlxG.random.float(0, FlxMath.remapToRange(MechanicManager.mechanics['burst_note'].points, 0, 20, 0, 2)) * 0.4;
					}
					if (FlxG.random.bool((((Math.min(MechanicManager.mechanics['note_random'].points * FlxMath.remapToRange(section.lengthInSteps, 0, 16, 1,
						6) / noteData.length * 0.2, 1))
						+ getChance(3)) * 1.1) * hitSectionMulti))
					{
						var randomNote:Note = new Note(sectionStartTime + (Conductor.stepCrochet * i), FlxG.random.int(0, 3), null, false);

						randomNote.strumTime -= 2; // prioritize chart notes
						randomNote.autoGenerated = true;
						randomNote.earlyHitMult = 0.4;
						randomNote.mustPress = randomNote.formerPress = j;
						if (playBothMode)
							randomNote.mustPress = true;
						randomNote.noteType = 'No Animation';
						randomNote.scrollSpeed = songSpeed;
						randomNote.scrollFactor.set();
						unspawnNotes.push(randomNote);
						spawnedNotes.push(randomNote);
						weightedChances[3] = 0;
					}
					else
					{
						weightedChances[3] += (FlxG.random.float(0,
							FlxMath.remapToRange(MechanicManager.mechanics['note_random'].points, 0, 20, 0, 4)) * 0.6) * hitSectionMulti;
					}
				}
			}
		...
		{
			FlxArrayUtil.clearArray(dumbNote.sustainChilds);
		...
		new FlxTimer().start(0.1, function(tmr:FlxTimer)
		{
			if (!paused && !startingSong && !endingSong && health > 0.1)
			{
				if (MechanicManager.mechanics['drain_hp'].points > 0)
				{
					...
					{
						health -= FlxMath.remapToRange(0.1, 0, 100, 0, 2);
					}
				}
				else
				{
	...
	private var burstTime:{min:Float, max:Float, value:Float} = {min: 0, max: 5, value: 3};
	...
	override public function update(elapsed:Float)
	{
		...
		if (MechanicManager.mechanics['burst_note'].points > 0)
		{
	 ...
	 */
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (cpuControlled) // if its botplay no need to check some massive statements
				{
				...
				else if (daNote.alpha != 0 && daNote.visible)
				{
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
			...
			&& (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			if (!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				if (possibleNotes.length > 0)
				{
					...
					for (coolNote in possibleNotes)
					{
						...
						if (coolNote.hitCausesMiss && coolNote.autoGenerated && possibleNotes[possibleNotes.indexOf(coolNote) + 1] != null)
							continue;
			...
			var spr:StrumNote = groupStrums.members[key];
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (note.noteType == 'No Animation' && note.autoGenerated)
		{
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (note.hitCausesMiss)
			{
				...
				if (note.autoGenerated) // to avoid conflicts with mods
				{
			...
			if (burstTime.value < burstTime.min || MechanicManager.mechanics['burst_note'].points == 0)
			...
			else
			{
				...
				groupStrums.forEach(function(spr:StrumNote)
				{
			...
			var formerPressed:Bool = note.mustPress;
			if (playBothMode)
