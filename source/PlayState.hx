...
class PlayState extends MusicBeatState
{
	...
	public var madnessOpponentText:Array<String> = [
		'unacceptable', 'failure', 'insufficient', 'invalid', 'incorrect', 'adjusting', 'misjudged', 'error', 'correction', 'improbable', 'mistake',
		'incomplete',
	];
	public var madnessMissText:Array<String> = [
		"terrible", "waste", "miss calculated", "predicted", "failure", "disgusting", "abhorrent", "foreseen", "contemptible", "prognosticate", "despicable",
		"reprehensible"
	];
	...
	override public function create()
	{
		...
		if (SONG.stage == null || SONG.stage.length < 1)
		{
			switch (songName)
			{
				...
				case 'Improbable-Outset' | 'Madness':
					curStage = 'madness-combat';
				case 'Hellclown':
					curStage = 'madness-combat-hellclown';
				default:
					curStage = 'stage';
			}
		}
		...
		switch (curStage)
		{
			...
			case 'madness-combat':
				...
				var bg:BGSprite = new BGSprite(['red', 'week-madness'], -350, -300, 0.35, 0.35);

				redBeatSprite = new FlxSprite(-40, -40).makeGraphic(Std.int(FlxG.width * 1.2), Std.int(FlxG.height * 1.2));
				redBeatSprite.color = 0xFF851E1E;
				redBeatSprite.alpha = 0.0;
				redBeatSprite.cameras = [camHUD];
				...
				{
					add(bg);
					stageFront = new BGSprite(['island_but_dumb', 'week-madness'], -1100, -460, 0.9, 0.9);
				}
				else
					stageFront = new BGSprite(['island_but_rocks_float', 'week-madness'], -1100, -460, 0.9, 0.9);

				stageFront.scale.set(1.4, 1.4);
				add(stageFront);

				var mainLight:BGSprite = new BGSprite(['hue', 'week-madness'], -470, -150, 1.2, 1.2);
				mainLight.alpha = 0.3;
				add(mainLight);
		...
		for (listed in mouseList)
		{
			...
			{
				mouseCursor = new FlxSprite().loadGraphic(Paths.image('cursor' + (hudType != '' ? '-$hudType' : '')));
			...
			{
				FlxG.mouse.load(new FlxSprite().loadGraphic(Paths.image('cursor' + (hudType != '' ? '-$hudType' : ''))).pixels);
				break;
			}
		}
	...
	private var introTextCustom:Map<String, Array<Dynamic>> = [
		'comedown' => ['Comedown', 'A loss of status or importance'], // week 1
		'solitude-denouement' => [
			'Solitude Denouement',
			'The state of isolation\n\nThe main outcome of a main dramatic complication'
		] // week 6
	];
	...
	public function startCountdown():Void
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			if (introTextCustom.exists(songPath) && deathCounter <= 0)
			{
				var blackScreen:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
				blackScreen.cameras = [camOther];
				add(blackScreen);

				var titleText:FlxText = new FlxText(0, 0, 0, introTextCustom[songPath][0], 64);
				titleText.setFormat(Paths.font("vcr.ttf"), 64, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
				titleText.borderSize = 2.5;
				titleText.screenCenter();
				titleText.cameras = [camOther];
				titleText.alpha = 0.0;
				add(titleText);

				var descText:FlxText = new FlxText(0, 0, 0, introTextCustom[songPath][1], 24);
				descText.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
				descText.borderSize = 2.5;
				descText.screenCenter();
				descText.y = titleText.y + titleText.height + 10;
				descText.alpha = 0.0;
				...
				{
					FlxTween.tween(titleText, {alpha: 1.0}, 1.5, {
						ease: FlxEase.quadOut,
						onComplete: function(twn:FlxTween)
						{
							new FlxTimer().start(1.25, function(tmr:FlxTimer)
							{
								FlxTween.tween(descText, {alpha: 1.0}, 1.5, {
									ease: FlxEase.quadOut,
									onComplete: function(twn:FlxTween)
									{
										new FlxTimer().start(2, function(tmr:FlxTimer)
										{
											FlxTween.num(1, 0, 1.0, {ease: FlxEase.quadOut}, function(v:Float)
											{
												blackScreen.alpha = titleText.alpha = descText.alpha = v;
											});
										});
									}
								});
				...
				// cheap way to prevent the song from playing
				FlxTween.num(0, 10, 8.25, {ease: FlxEase.quadOut}, function(v:Float)
				{
	...
	function startSong():Void
	{
		...
		FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song), ClientPrefs.musicVolume / 10, false);
	...
	private function sleepNote():Void
	{
		...
		else
		{
			...
			mechanicsResult[5].value += random;
	...
	private function failedDodge():Void
	{
		...
		if (mechanicsResult[11] != null)
			mechanicsResult[11].value += 1;
	}
	...
	private function updateTimeMechanic()
	{
		...
		if (Conductor.songPosition - 1500 >= timeNeed) // +0.5 sec for pacifist
		{
			...
			timeAttempts++;
			changeMorale(0.9);
			if (mechanicsResult[15] != null)
				mechanicsResult[15].value += 1;
		}
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			case 'madness-combat' | 'madness-combat-hellclown':
				for (object in madnessTextList)
				{
					object.updateTime += elapsed;
					while (object.updateTime >= 1 / 60)
					{
						object.updateTime -= 1 / 60;
						object.text.angle = FlxG.random.float(-15, 15);
					}

					if ((object.lifeTime += elapsed) >= 0.3)
					{
						object.text.kill();
						object.text.destroy();
						...
						madnessTextList.remove(object);
					}
				}

				if (madnessTextList.length > 0)
				{
					camHUD.alpha = 0.8;
				}
				else
	 ...
	 */
		var stringText:String = ScoreText.generateText(songScore, songMisses, ratingName, CoolUtil.floorDecimal(ratingPercent * 100, 2), ratingFC);

		if (AIPlayer.active)
		{
			stringText = '<win>' + stringText + '<win>';
			...
			var aiText:String = '\n [AI] ' + ScoreText.generateScore(AIScore) + ' ' + ScoreText.generateMiss(AIMisses) + ' '
				+ ScoreText.generateRating(AIRank, AIAccuracy);

			if (AIAccuracy > CoolUtil.floorDecimal(ratingPercent * 100, 2))
			{
				stringText = stringText.replace('<win>', '');
				aiText = '<win>' + aiText + '<win>';
			}

			stringText += aiText;

			scoreTxt.text = stringText;

			if (_oldText != scoreTxt.text)
			{
				var goldFormat:FlxTextFormat = new FlxTextFormat(0xFFFFD900, false, false, 0xFF000000);
				scoreTxt.applyMarkup(stringText, [new FlxTextFormatMarkerPair(goldFormat, '<win>')]);

				_oldText = scoreTxt.text;
			}
		}
		else
			scoreTxt.text = stringText;
		...
		{
			/*
			var glitchText:GlitchyText = new GlitchyText(FlxG.random.float(100, 1000), FlxG.random.float(30, 630));
			glitchText.cameras = [camHUD];
			add(glitchText);
		 */
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			...
			case 'Kill Henchmen':
				killHenchmen();
			case 'Advanced Camera Tween': // only used for mechanics mod port, i will not fix stuff if it is broken for your mod
				var cameraIndex:Array<String> = value1.split(',');
				var tweenIndex:Array<String> = value2.split(',');

				var getFlxEaseByString:String->Dynamic = function(ease)
				{
				...
				var targetX:Float = Math.isNaN(Std.parseFloat(cameraIndex[0])) ? 0.0 : Std.parseFloat(cameraIndex[0]);
				var targetY:Float = Math.isNaN(Std.parseFloat(cameraIndex[1])) ? 0.0 : Std.parseFloat(cameraIndex[1]);
				var targetAngle:Float = Math.isNaN(Std.parseFloat(cameraIndex[2])) ? 0.0 : Std.parseFloat(cameraIndex[2]);
				var targetZoom:Float = Math.isNaN(Std.parseFloat(cameraIndex[3])) ? 1.0 : Std.parseFloat(cameraIndex[3]);
				...
				FlxTween.tween(FlxG.camera, {
					...
					angle: targetAngle,
					zoom: targetZoom
				}, Std.parseFloat(tweenIndex[0]), {
					ease: getFlxEaseByString(tweenIndex[1]),
					onComplete: function(twn:FlxTween)
					{
						camZoomingMult = 1.0;
						camZoomingDecay = 1.0;
			...
			case 'Persistent Draw':
				FlxG.state.persistentDraw = !FlxG.state.persistentDraw;
				trace(FlxG.state.persistentDraw);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		vocals.volume = ClientPrefs.vocalVolume / 10;
		...
		else
		{
			switch (daRating)
			{
				...
				case "good":
					{
						AITotalNotesHit += 0.75;
						score = 200;
					}
				case "bad":
					{
						AITotalNotesHit += 0.50;
						score = 100;
					}
				case "shit":
					{
						AITotalNotesHit += 0.25;
						score = 50;
					}
			}
		...
		if (!practiceMode /* && !cpuControlled */)
		{
			...
			{
				if (!note.mustPress)
					AIScore += Math.ceil(score * MechanicManager.multiplier);
				else
	...
	{ // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (hudType == 'madness' && FlxG.random.bool(combo / 7))
		{
			...
			var madnessText:FlxText = new FlxText(0, 0, 0, FlxG.random.getObject(madnessMissText).toUpperCase(), 78);
			madnessText.setFormat(Paths.font("impact.ttf"), 78, fullColor, CENTER, OUTLINE, outlineColor);
			madnessText.borderSize = 2.5;
			madnessText.cameras = [camOther];

			var trickyMiddle = dad.getScreenPosition(null, camOther);

			trickyMiddle.x += dad.getGraphicMidpoint().x - dad.x;
			trickyMiddle.y += dad.getGraphicMidpoint().y - dad.y;

			madnessText.setPosition(trickyMiddle.x, trickyMiddle.y);
			add(madnessText);
			madnessTextList.push({text: madnessText, lifeTime: 0.0, updateTime: 0.0});
		}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		else if (!note.noAnimation)
		{
			...
			if (!note.isSustainNote)
			{
				if (hudType == 'madness' && FlxG.random.bool(6))
				{
					var fullColor = FlxColor.interpolate(FlxColor.RED, FlxColor.BLACK,
						Math.min(FlxMath.remapToRange(Conductor.songPosition, 0, FlxG.sound.music.length * 0.8, 0, 1), 1));
					var outlineColor = FlxColor.interpolate(FlxColor.BLACK, FlxColor.RED,
					...
					var madnessText:FlxText = new FlxText(0, 0, 0, FlxG.random.getObject(madnessOpponentText).toUpperCase(), 78);
					madnessText.setFormat(Paths.font("impact.ttf"), 78, fullColor, CENTER, OUTLINE, outlineColor);
					madnessText.borderSize = 2.5;
					madnessText.cameras = [camOther];
	...
	function opponentMiss(daNote:Note):Void
	{
		...
		if (!practiceMode)
			AIScore -= Math.ceil(25 * MechanicManager.multiplier);
	}
