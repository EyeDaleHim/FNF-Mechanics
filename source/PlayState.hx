...
class PlayState extends MusicBeatState
{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var noteGroup:{hittableNotes:Array<Note>, enemyNotes:Array<Note>} = {hittableNotes: [], enemyNotes: []};
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (swagNote.canBeHit)
				{
					noteGroup.hittableNotes.push(swagNote);
				}
				else
				{
					noteGroup.enemyNotes.push(swagNote);
				}
			}
			...
			for (i in 0...section.lengthInSteps)
			{
					...
					+ getChance(0)))
				{
					...
					hurtNote.mustPress = FlxG.random.bool();
					...
					+ getChance(1)) * 0.5))
				{
					...
					killNote.mustPress = FlxG.random.bool();
					...
					+ getChance(2)) * 0.9))
				{
					...
					burstNote.mustPress = FlxG.random.bool();
		...
		for (note in spawnedNotes)
		{
			if (note.mustPress)
			{
				for (songNote in noteGroup.hittableNotes)
				{
					if (Math.abs(songNote.strumTime - note.strumTime) < 10 && songNote.noteData == note.noteData)
					{
						dupeNotes.push(note);
					}
				}
			...
			{
				for (songNote in noteGroup.enemyNotes)
				{
					if (Math.abs(songNote.strumTime - note.strumTime) < 10 && songNote.noteData == note.noteData)
					{
						dupeNotes.push(note);
		...
		FlxArrayUtil.clearArray(noteGroup.hittableNotes);
		FlxArrayUtil.clearArray(noteGroup.enemyNotes);
	...
	private var burstTime:{min:Float, max:Float, value:Float} = {min: 0, max: 5, value: 0};
